...
"\n$0: version $version\n\nUSAGE:\n\n  $0 pm###x [all met wind at rh swr rain lwr baro temp sal pres curr]\n\n";
    ...
    $ramdbloadlog = File::Spec->join( $ENV{RAMDBLOADLOGDIR},
        "RAMdbload" . $dep . "_out.txt" );
    ...
    foreach (@dir_list) {
        if (
/^(met|rad|lwr|baro|rain|sal[cms]|pres|temp|curr)[0-9]?$dep2\.davg$/i
          )
        {
            push @files, $_;
        }
    ...
    if ( @files == 0 ) { warn "No files found in $ram_dir!\n"; }
...
## should process all files!
    ...
    else { print "No cal file being loaded!!!\b\n"; }

    if ($buoy) {
        @all_sensors =
          keys %{ $buoy->{sensor_label} };    #used later to get sn, depth etc
    ...
    if ( scalar(@files) == 0 ) { print "no data files found!\n"; }
    ...
    foreach my $file (@files) {
        ...
        my @head;
        for my $line ( 0 .. 4 ) { push @head, scalar <$fh> }

        #if no buoy location from cal file, try to get it from the data file
        unless ($site) { $site = get_site( $head[1] ); }
   ...
   # it doesn't really matter what is used, just a question of convenience
    ...
    #                This is a _list_; wind has 2 qualities
        ...
        my (
            %seacat_type,   %seacat_serial,   %module_type,
            %module_serial, %sal_type_column, @depth
        );
    ...
    #                    the "senstype" which we should use
        ...
        if ( $file =~ m/met/io ) {
            unless ( @{ $data[0] } == 15 ) { die "$warn1 MET $warn2"; }
            ...
            if ( $opts =~ /wind/io ) {
                ...
                @{ $source_column{wind} } =
                  (5);    #ignore the second one, always the same
                @{ $qc_columns{wind} } = ( 4, 7 );
            }
            if ( $opts =~ /at|air/io ) {
                $table{temp}  = "temp";
            ...
            unless ( @{ $data[0] } == 6 ) { die "$warn1 SWR $warn2"; }
            $table{swr}  = "swr";
            ...
            unless ( @{ $data[0] } == 6 ) { die "$warn1 RAIN $warn2"; }
            $table{rain}  = "rain";
            ...
            unless ( @{ $data[0] } == 5 ) { die "$warn1 LWR $warn2"; }
            $table{lwr}  = "lwr";
            ...
            unless ( @{ $data[0] } == 4 ) { die "$warn1 BARO $warn2"; }
            $table{baro}  = "baro";
          ...
          #data files:
            @sensors =
              sort { $buoy->get_depth($a) <=> $buoy->get_depth($b) } @sensors;

            if ( scalar(@sensors) != $#{ $data[0] } / 3 ) {
                print "number of TEMP sensors ("
                  . scalar(@sensors) . ") "
                  . "in cal file does not match data file ("
                  . ( $#{ $data[0] } / 3 )
                  . ") !\n\n";
 ...
 #  So, we get info ONLY from data file, not from cal file (buoy object) at all.
            ...
            %seacat_type =
              map { ( $depth[$_], $seacat_type[$_] ) } ( 0 .. $#depth );
            %seacat_serial =
              map { ( $depth[$_], $seacat_serial[$_] ) } ( 0 .. $#depth );
            %module_type =
              map { ( $depth[$_], $module_type[$_] ) } ( 0 .. $#depth );
            %module_serial =
              map { ( $depth[$_], $module_serial[$_] ) } ( 0 .. $#depth );
            undef @seacat_type;
            undef @seacat_serial;
            undef @module_type;
            undef @module_serial;
 ...
 #  So, we get info ONLY from data file, not from cal file (buoy object) at all.
            ...
            foreach (
                @depth)  # current file has 4 identical depth headers per sensor
            {
...
#    my @serial = $head[2] =~ m/ARG\s*(\d+)/og;
            ...
            foreach (@depth)    #have to use array to keep in depth order
            {
                ...
                @{ $source_column{$_} } =
                  ( $data_column + 4 ); #ignore the second one; always the same?
...
#      $type{$_}             = 40; # make this for later use like other files
          ...
          #data files:
            ...
            if ( @sensors != $#{ $data[0] } / 3 ) {
                die "number of PRES sensors in cal file ("
                  . scalar(@sensors)
        ...
        else { die "Unidentifiable file type $file!"; }
        ...
        foreach ( keys %sensor ) {
            ...
            if ( $file =~ m/curr/io )    #currents
            {
                ...
                my $rows = print $dbh (
                    qq{ UPDATE $table{$_} SET use_site=0
                              WHERE deploy_id = '$dep'
                              AND (senstype=$type{$_} AND sensor_sn=$serial{$_})
                              AND data_mode < 5\n}
                );

                unless ( defined $rows ) { warn("Bad update!") }
...
#now loop over each column (temperatures by depth, for instance, or wind, at, rh)
          ...
          # so this basically says "if we have any good data..."
              ...
              #for the seperate seacat and module salinity files, DON'T do this!
                    ...
                    {    # realtime for currents is updated per deployment
                        $update = undef;

                    }
                    elsif ( $table{$sensor} =~ /sal/io ) {
                        if ( $file =~
                            /salc/io ) # don't set real-time use=0 for s,m files
                        {
                            $update =
                ...
                else #we have bad new data, "use" only if real time bad or nonexistent
                {
                    ...
                    # for everything BUT wind and sal, only one datum
                        unless ( $table{$sensor} =~ /wind|sal/io ) {
                            $columns = "data_avg";
                            $where_clause =
                                "WHERE  obs_time = DATE_ADD(\"$jan1date\", "
                              . "INTERVAL $days_to_add DAY) "
                              . "AND  deploy_id = \"$dep\" "
                        ...
                        my $query =
                          "SELECT $columns FROM $table{$sensor} $where_clause "
                          . "ORDER BY obs_time \n";
                        my $sth  = print $dbh $query;
                        my $rows = 0;

                        if ( defined $rows
                            and $rows == 0 )    #returned "0E0", no rows found
                        { $use = 1; }
      ...
      #  $sth->finish();
          ...
          # to add the days of the year.
             ...
             # So we just use "REPLACE" for all.
                ...
                unless ( $file =~ /sal/io ) {
                    $insert =
                        "REPLACE INTO $table{$sensor} VALUES(\"$site\", "
           ...
           # and something like "2,NULL,2,NULL" for wind.
                ...
                {
                    my $sample_type =
                      @line[ $sal_type_column{$sensor} ];    #24 or 70/71
                    my $serial;
                    ...
                    if ( $module_type{$sensor} == $sample_type ) {
                        ...
                        $serial = $seacat_serial{$sensor} . ":"
                          . $module_serial{$sensor};
                    }
                    elsif ( $sample_type == 0 ) { }
                    else                        { die "whoops. problem here!"; }

                    unless ( $sample_type ==
                        0 )    #if we got a "00" type sample, skip it
                    {
                        $insert =
                            "REPLACE INTO $table{$sensor} VALUES(\"$site\", "
                          . "DATE_ADD(\"$jan1date\", INTERVAL $days_to_add DAY), "
                ...
                if ($insert) {
                    ...
                    #print "$insert\n";
                    unless ( defined $rows ) { die("Bad insert!") }
                    if     ( $rows == 1 or $rows == 2 )
                    ...
                    { $ram_rows_changed++; }
                    else { die "bad number of rows changed"; }
...
sub get_site {
    ...
    my ( $latd, $latm, $NorS, $lond, $lonm, $EorW ) =
      ( $line =~
/DEPLOYED AT\s+(\d+)\s+(\d+.?\d*)([NnSs]?)\s?,\s?(\d+)\s+(\d+.?\d*)([EeWw]?)/io
      );
...
sub is_deployed {
    ...
    # issue query
    my $sth = $dbh->prepare(
        qq{SELECT COUNT(*) from LoadStatus WHERE deploy_id='$deplid'})
    ...
    if ( -r $scriptfile ) {
        ...
        foreach (@scriptlines) {
            ...
            if ( $sqlcommand =~ /;\s*$/ ) {
                ...
                else { print "$rows rows changed: $sqlcommand\n"; }
